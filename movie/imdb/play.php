<?php
  ...
  if (isset($_GET['id'])) 
    ...
    $post_id = $_GET['id'];
    $return = 'index.php';
    $sql = $conn->prepare("SELECT * FROM `movie` WHERE `imdb`='$post_id'");
    $sql->execute();
...
if ($sql->rowCount() > 0) {
echo  "<iframe src=\"{$first}\" style=\"background: #fff;\" frameborder=\"0\" height=\"100%\" scrolling=\"auto\" width=\"100%\" allowfullscreen></iframe>";
} else {
...
<html data-fp="xpgvjo5qknj"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8"><script type="text/javascript" src="./src/moatframe.js.download"></script>
    ...
    <div class="media-title">
        <div class="show-name"><?php echo $title;?></div>
    </div>
                </div>
            </div>
        </div>
